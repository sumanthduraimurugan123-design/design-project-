...
export default function Globe3D({ selectedCountry = 'global', onSelectCountry }) {
  ...
  return (
    <div className="relative w-full h-[460px] lg:h-[520px] overflow-hidden border border-wire-border flex flex-col bg-[#0A0908]">
      
      {/* Top Left HUD — station ID */}
      <div className="absolute top-2.5 left-3 z-10 flex flex-col sm:flex-row items-start sm:items-center gap-2">
        <div className="flex items-center gap-2 bg-wire-base/90 backdrop-blur-sm px-2.5 py-1.5 border border-wire-border">
          <Crosshair className="w-3.5 h-3.5 text-wire-amber" />
          <span className="font-mono text-[10px] text-wire-fg tracking-widest">ORBITAL SENTINEL</span>
          <span className="font-mono text-[9px] text-wire-subtle border border-wire-border px-1.5 py-0.5">
            {globeMode === 'space' ? 'Daylight' : 'Night'}
          </span>
        </div>

        {/* View toggle */}
        <div className="flex items-center bg-wire-base/90 backdrop-blur-sm border border-wire-border font-mono text-[10px]">
          <button
      ...
      <div className="absolute top-2.5 right-3 z-10 flex items-center gap-1 bg-wire-base/90 backdrop-blur-sm p-1 border border-wire-border">
        <button
          onClick={() => setHeatmapGlow(!heatmapGlow)}
          className={`px-2 py-1 font-mono text-[10px] flex items-center gap-1 transition-colors ${
            heatmapGlow ? 'text-wire-amber' : 'text-wire-subtle hover:text-wire-fg'
          }`}
          title="Toggle heatmap rings"
        >
      ...
      {/* Bottom hotspot bar */}
      <div className="absolute bottom-0 left-0 right-0 z-10 flex flex-wrap items-center gap-1 bg-wire-base/90 backdrop-blur-sm px-3 py-2 border-t border-wire-border">
        <span className="font-mono text-[10px] text-wire-subtle mr-1">Hotspots:</span>
        {GLOBAL_HOTSPOTS.slice(0, 7).map(h => (
          <button
            key={h.id}
            onClick={() => onSelectCountry(h.id)}
            className={`font-mono text-[10px] px-2 py-0.5 border transition-colors flex items-center gap-1 ${
              selectedCountry.toLowerCase() === h.id
                ? 'bg-wire-raised text-wire-fg border-wire-muted'
